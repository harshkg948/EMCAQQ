 ...
 * Body: { email: string, password: string, name?: string, gender?: string, age?: number, dob?: string }
 */
export async function register(c: Context) {
    try {
        const body = await c.req.json();
        const { email, password, name, gender, age, dob } = body;
        ...
        const user = await userService.registerWithEmailPassword(email, password, name, gender, age, dob);
...
/**
 * PATCH /auth/assessment
 * Update user assessment data
 */
export async function updateAssessment(c: Context) {
    const userId = c.get('userId');
    const body = await c.req.json();
